...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Retry failed instances from errors.json (one-shot mode)')
    ...
    output_dir_name = f"outputs_oneshot_retry_{starter_time}"
    info_saving_json_path = os.path.join(
        args.info_saving_dir_path,
        f'intermediate_metadata_oneshot_retry_{starter_time}.json'
    )
    ...
    for pending_index, pending_item in enumerate(pending_items):
        ...
        try:
            ...
            if should_stop_on_error(exc):
                ...
                save_json(info_saving_json_path, generation_results)
                raise RuntimeError(
                    f"One-shot retry stopped on {task_instance_id}: {exc}\n"
